...
class UsuariosModel extends Mysql
{
    ...
    {
        //VALIDAMOS SESION PARA ASIGNAR EL SUPER USUARIO
        $whereAdmin = "";
        if ($_SESSION['idUser'] != 1) {
            $whereAdmin = " and p.idpersona != 1 ";
        }
        $sql = "CALL SP_LISTAR_USUARIOS()" . $whereAdmin;
        $request = $this->select_all($sql);
        return $request;
    }
    ...
    public function updateUsuario(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, int $tipoid, int $status)
    {
        ...
        //VALIDAMOS SI AL ACTUALIZAR SE PONE UN CORREO QUE YA EXISTE PARA OTRO USUARIO SI NO SE ACTUALIZA CORRECTAMENTE.
        ...
        if (empty($request)) {
            //VALIDMOS SI ACTUALIZAMOS PASSWORD SE EJECTA ESTE UPDATE
            if ($this->strPassword  != "") {
